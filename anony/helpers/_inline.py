...
def leaderboard_markup(_lang: dict, chat_id: int) -> types.InlineKeyboardMarkup:
    return ikm(
        [
            [
                ikb(text=_lang["ld_users"], callback_data=f"leaderboard users {chat_id}", style=enums.ButtonStyle.PRIMARY),
                ikb(text=_lang["ld_groups"], callback_data="leaderboard groups", style=enums.ButtonStyle.PRIMARY),
            ]
        ]
    )


def leaderboard_period_markup(_lang: dict, chat_id: int) -> types.InlineKeyboardMarkup:
    return ikm(
        [
            [
                ikb(text=_lang["ld_daily"], callback_data=f"leaderboard period {chat_id} daily"),
                ikb(text=_lang["ld_weekly"], callback_data=f"leaderboard period {chat_id} weekly"),
                ikb(text=_lang["ld_overall"], callback_data=f"leaderboard period {chat_id} overall"),
            ],
            [
                ikb(text=_lang["back"], callback_data=f"leaderboard back {chat_id}", style=enums.ButtonStyle.PRIMARY),
                ikb(text=_lang["close"], callback_data="leaderboard close", style=enums.ButtonStyle.DANGER),
            ],
        ]
    )
...
def leaderboard_result_markup(
    _lang: dict, chat_id: int, groups: bool = False
) -> types.InlineKeyboardMarkup:
    rows = []
    if groups:
        rows.append([ikb(text=_lang["ld_support"], url=config.SUPPORT_CHAT)])
    rows.append(
        [
            ikb(
                text=_lang["back"],
