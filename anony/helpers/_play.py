...
async def ensure_play_permissions(m: types.Message, force: bool = False) -> bool:
    chat_id = m.chat.id
    ...
    if play_mode or force:
        ...
        if (
            ...
            await m.reply_text(m.lang["play_admin"])
            return False
...
def checkUB(play):
    async def wrapper(_, m: types.Message):
        ...
        if not await ensure_play_permissions(m, force):
            return
